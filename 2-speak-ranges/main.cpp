...
int main() {
  std::vector<Person> people{
      {29, "Jack"},   {32, "Elise"},  {66, "Jennifer"}, {69, "Paul"},
      {30, "Hannah"}, {31, "Dennis"}, {4, "Veronica"},  {1, "Helena"},
  };

  if (auto it = std::ranges::find(people, std::string("Hannah"), &Person::name);
      it != people.end()) {
  ...
  auto ageDivisibleBy = [](int d) {
    return [d](int age) { return age % d == 0; };
  };
